...
test.describe('Critical User Paths', () => {
  ...
  test('user journey: Landing → SATOR Hub → Prediction → Result', async ({ page }) => {
    ...
    await page.goto('/')
    await expect(page).toHaveTitle(/EXE|TENET/)
  ...
  test('offline scenario: Disconnect → Cached data → Reconnect', async ({ page }) => {
    ...
    await expect(page.getByText(/EXE|TENET|SATOR|Platform/i).first()).toBeVisible()
    ...
    const content = page.getByText(/Observatory|SATOR|Offline|Cached|EXE/i).first()
