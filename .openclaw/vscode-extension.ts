...
import { VSCode:ACPClient, createKimiACPClient, createBridgeACPClient } from "./vscode-acp-client";

let acpClient: VSCode:ACPClient | undefined;
    ...
    await vscode.window.withProgress(
      ...
      async (progress, token) => {
        ...
        vscode.window.showInformationMessage(
          `Agent completed: ${response.stopReason}`
        );
      }
    );
  } catch (error) {
...
class ACPSessionTreeProvider implements vscode.TreeDataProvider<ACPTreeItem> {
  private _onDidChangeTreeData = new vscode.EventEmitter<ACPTreeItem | undefined>();
  readonly onDidChangeTreeData = this._onDidChangeTreeData.event;

  getTreeItem(element: ACPTreeItem): vscode.TreeItem {
    return element;
  }

  getChildren(element?: ACPTreeItem): Thenable<ACPTreeItem[]> {
    if (!element) {
      // Root level
      return Promise.resolve([
        new ACPTreeItem(
          "Status",
          acpClient?.isConnected() ? "Connected" : "Disconnected",
        ...
        new ACPTreeItem(
          "Session",
          acpClient?.getSessionId() || "No active session",
          vscode.TreeItemCollapsibleState.None
        ),
      ]);
    }
