...
from .decomposition import RARDecomposer
from .age_curves import compute_age_curve
...
class InvestmentGrader:
    ...
    def batch_grade(
        ...
        Args:
            players: List of dicts with keys: raw_rating, role, age, record_date
            
        Returns:
            List of grade results with player_id included
        """
        results = []
        for player in players:
            result = self.grade(
                raw_rating=player.get("raw_rating", 0.0),
                role=player.get("role", "Entry"),
                age=player.get("age", 25),
                peak_age_range=player.get("peak_age_range"),
                record_date=player.get("record_date")
            )
            result["player_id"] = player.get("player_id", "unknown")
