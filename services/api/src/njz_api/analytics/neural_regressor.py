...
class SeasonCohortNormalizer:
    ...
    def compute_player_zscores(
        ...
        """
        Compute z-scores for a single player against cohort averages.
        
        Args:
            player_stats: Dict of player statistics
            cohort_stats: Dict with 'mean' and 'std' for each stat
        ...
        for stat in NUMERIC_COLS:
            if stat in player_stats and stat in cohort_stats.get("mean", {}):
                value = player_stats[stat]
                mean = cohort_stats["mean"][stat]
                std = cohort_stats["std"][stat]
                z_scores[f"{stat}_z"] = (value - mean) / (std + 1e-8) if std > 0 else 0
        return z_scores
