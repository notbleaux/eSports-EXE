...
def compute_confidence_tier(
    games_played: int,
    recent_form_consistency: float,  # 0-1
    data_quality_score: float  # 0-1
) -> int:
    """
    Compute confidence tier (0-100) for a player record.
    Higher = more confident in the rating.
    """
    # Games played component (max 40 points)
    games_component = min(games_played / 50, 1.0) * 40
    
    # Consistency component (max 35 points)
    consistency_component = recent_form_consistency * 35
    
    # Data quality component (max 25 points)
    quality_component = data_quality_score * 25
    
    total = int(games_component + consistency_component + quality_component)
