...
class SimRatingCalculator:
    ...
    def calculate_percentile(self, sim_rating: float) -> float:
        """
        Convert SimRating z-score to percentile.
        Assuming normal distribution with mean 0, std 1 for z-scores.
        """
        from scipy.stats import norm
        percentile = norm.cdf(sim_rating) * 100
        return round(percentile, 1)

    def interpret_rating(self, sim_rating: float) -> str:
        """Human-readable interpretation of SimRating."""
        if sim_rating >= 2.0:
            return "Elite - Top 2% performer"
        elif sim_rating >= 1.5:
            return "Excellent - Top 7% performer"
        elif sim_rating >= 1.0:
            return "Very Good - Top 16% performer"
        elif sim_rating >= 0.5:
