...
Usage:
    python dashboard.py --db-url postgresql://... --coordinator http://localhost:8080
"""
import asyncio
import argparse
...
class PipelineDashboard:
    ...
    async def _get_agent_overview(self) -> Dict:
        ...
        async with self.db_pool.acquire() as conn:
            agents = await conn.fetch("""
                SELECT 
                    id, status, current_job_id, last_heartbeat,
            ...
            return {
                "cs": {
                    ...
                    "by_source": [
                        {
                            "source": s['source'],
                            "last_extraction": s['last_extraction'].isoformat(),
                            "records_24h": s['records_24h']
                        }
    ...
    async def get_detailed_queue(self, game: Optional[str] = None) -> List[Dict]:
        ...
        async with self.db_pool.acquire() as conn:
            query = """
                SELECT 
                    id, game, source, job_type, priority, status,
                    epoch, region, created_at, started_at,
                    assigned_agent, retry_count, error_message
                FROM extraction_jobs
    ...
    async def get_failed_jobs(self, game: Optional[str] = None, hours: int = 24) -> List[Dict]:
        ...
        async with self.db_pool.acquire() as conn:
            query = """
                SELECT 
                    id, game, source, job_type, completed_at,
                    retry_count, error_message, assigned_agent
                FROM extraction_jobs
                WHERE status = 'failed'
        ...
        async function loadData() {
            try {
                ...
                document.getElementById('content').innerHTML = 
                    '<p class="critical">Error loading data: ' + e.message + '</p>';
            }
        }
            ...
            document.getElementById('content').innerHTML = `
                <div class="grid">
                    <div class="card">
                        ...
                        <div class="metric"><span>Pending</span><span class="metric-value">${cs.pending}</span></div>
                        <div class="metric"><span>Processing</span><span class="metric-value">${cs.processing}</span></div>
                        <div class="metric"><span>Completed (24h)</span><span class="metric-value">${cs.completed_24h}</span></div>
                    </div>
                    <div class="card">
                        ...
                        <div class="metric"><span>Pending</span><span class="metric-value">${val.pending}</span></div>
                        <div class="metric"><span>Processing</span><span class="metric-value">${val.processing}</span></div>
                        <div class="metric"><span>Completed (24h)</span><span class="metric-value">${val.completed_24h}</span></div>
                    </div>
                    <div class="card">
                        <h2>Agents</h2>
                        <div class="metric"><span>Active</span><span class="metric-value">${data.agents.total_active}</span></div>
                        <div class="metric"><span>Idle (CS)</span><span class="metric-value">${data.agents.idle_cs}</span></div>
                        <div class="metric"><span>Idle (VAL)</span><span class="metric-value">${data.agents.idle_valorant}</span></div>
                        <div class="metric"><span>Busy</span><span class="metric-value">${data.agents.busy}</span></div>
                    </div>
                    <div class="card">
                        <h2>Conflicts</h2>
                        <div class="metric"><span>Open Conflicts</span><span class="metric-value">${data.conflicts.total_open}</span></div>
                        <div class="metric"><span>Duplicates</span><span class="metric-value">${data.conflicts.duplicates}</span></div>
                    </div>
                </div>
