...
if (-not $SkipSupabaseCheck) {
    ...
    Write-Host "  2. Locate project named 'ZeSporteXte' (or matching this repo/environment naming convention)"
    Write-Host "  3. If no matching project exists, create one first, then open it"
    Write-Host "  4. Click 'Resume project' and wait 2-3 minutes for database start"
    Write-Host ""
