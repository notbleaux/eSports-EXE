/**
 * @esportexe/websocket-client
 * Universal WebSocket client for eSports-EXE real-time match data (Path A Live Distribution)
 *
 * Connects to the EXE WebSocket service and broadcasts live match events.
 * Compatible with: Web App, Browser Extension, LiveStream Overlay.
 *
 * SCHEMA CHANGE: @esportexe/websocket-client package created — 2026-03-27
 */
...
export interface ExeWebSocketConfig {
...
export class ExeWebSocketClient {
  private ws: WebSocket | null = null;
  private reconnectAttempts = 0;
  private listeners = new Map<WsEventType | '*', Array<(msg: WsMessage) => void>>();
  private readonly config: Required<ExeWebSocketConfig>;

  constructor(config: ExeWebSocketConfig) {
    this.config = {
      autoReconnect: true,
...
export function createMatchClient(baseUrl: string, matchId: string): ExeWebSocketClient {
  const client = new ExeWebSocketClient({ baseUrl });
  client.connectMatch(matchId);
  return client;
}

export function createGlobalClient(baseUrl: string): ExeWebSocketClient {
  const client = new ExeWebSocketClient({ baseUrl });
